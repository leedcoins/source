...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZLDCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zldcTracker->HasMintTx(hash))) {
            //zLDC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZLDC;
...
bool IsZLDCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZLDC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZLDC || type == TransactionRecord::MNReward) {
