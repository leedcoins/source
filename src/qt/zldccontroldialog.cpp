...
#include "ui_zldccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zldcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zldcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLDC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLDC seed used to mint this zLDC is not the same as currently hold in the wallet";
            else
