...
CzLDCTracker::CzLDCTracker(std::string strWalletFile)
{
...
CzLDCTracker::~CzLDCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLDCTracker::Init()
{
...
bool CzLDCTracker::Archive(CMintMeta& meta)
{
...
bool CzLDCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLDCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLDCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLDCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLDCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLDCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLDCTracker::GetSerialHashes()
{
...
CAmount CzLDCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLDCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLDCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLDCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLDCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLDCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLDCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLDCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLDCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLDCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLDCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLDCWallet* zLDCWallet)
{
    bool iszLDCWalletInitialized = (NULL != zLDCWallet);
    CMintMeta meta;
    ...
    if (! iszLDCWalletInitialized)
        zLDCWallet = new CzLDCWallet(strWalletFile);
    meta.isSeedCorrect = zLDCWallet->CheckSeed(dMint);
    if (! iszLDCWalletInitialized)
        delete zLDCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLDCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLDCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLDCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLDCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLDCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLDCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLDCWallet* zLDCWallet = new CzLDCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLDCWallet);
        }
        delete zLDCWallet;
        LogPrint("zero", "%s: added %d dzldc from DB\n", __func__, listDeterministicDB.size());
    }
