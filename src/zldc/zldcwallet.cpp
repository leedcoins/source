...
CzLDCWallet::CzLDCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zldc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLDCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLDCSeed_deprecated()) {
                    LogPrintf("%s: Updated zLDC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zldc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zldc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLDCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLDCCount(nCountLastUsed);
    else if (!walletdb.ReadZLDCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLDCWallet::Lock()
{
    seedMaster = 0;
}

void CzLDCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLDCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLDC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLDCWallet::LoadMintPoolFromDB()
{
...
void CzLDCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLDCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLDCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zldcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLDCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLDC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zldcTracker which also adds to database
    pwalletMain->zldcTracker->Add(dMint, true);
...
void CzLDCWallet::SeedToZLDC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLDCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLDCWallet::GenerateDeterministicZLDC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLDCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLDCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLDCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
