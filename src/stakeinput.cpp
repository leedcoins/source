...
// The zLDC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLDC is a hash of the serial
...
{
    //Create an output returning the zLDC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLDCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLDC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLDC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLDCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLDC output", __func__);
...
{
    CzLDCTracker* zldcTracker = pwallet->zldcTracker.get();
    CMintMeta meta;
    if (!zldcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zldcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LDC stake is the outpoint
