...
public:
    ...
    bool WriteZLDCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLDCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLDCSeed_deprecated(uint256& seed);
    bool EraseZLDCSeed();
    bool EraseZLDCSeed_deprecated();

    bool WriteZLDCCount(const uint32_t& nCount);
    bool ReadZLDCCount(uint32_t& nCount);
